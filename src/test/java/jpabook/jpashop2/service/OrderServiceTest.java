...
public class OrderServiceTest {
    ...
    public void 상품주문() throws Exception{
        ...
        Item book = createBook("JPA 실전", 10000, 10);
    ...
    public void 상품주문_재고수량초과() throws Exception{
        //given
        Member member = createMember();
        Item item = createBook("JPA 실전", 10000, 10);

        int orderCount = 11;

        //when
        orderService.order(member.getId(), item.getId(), orderCount);
        //removeStock() 자체의 단위테스트 필요 -> ItemTest
    ...
    private Member createMember() {
        ...
        member.setName("회원1");
        member.setAddress(new Address("서울", "청계천로", "123-123"));
        em.persist(member);
        return member;
    }
}